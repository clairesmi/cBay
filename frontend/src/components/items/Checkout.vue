<template>
  <div id="checkout" class="flex flex-col items-center">
    <h1 class="animated zoomInDown checkout-title text-6xl tracking-wide text-orange-600 mt-10 mb-10">Checkout</h1>
    <h2 class="total">Total: £{{ total }}</h2>
    <div class="flex justify-center mt-6 w-full"><button class="w-1/4 button bg-red-100" @click="makePayment">Make Payment</button></div>
    <div class="flex justify-center mt-3 w-full"><router-link class="flex justify-center w-full" to="/basket"><button class="w-1/4 button bg-red-100">Back to Basket</button></router-link></div>
  </div>
</template>
<script>
