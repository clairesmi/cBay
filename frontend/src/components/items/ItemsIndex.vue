<template>
  <div
    id="items-index"
    class="flex flex-col justify-center items-center font-sans bg-red-200"
  >
    <h1
      class="animated zoomInDown listings-title tracking-wide text-orange-600"
    >
      All Listings
    </h1>
    <form class=" flex justify-center w-full text-gray-800">
      <input
        class="listings-search mt-5 w-1/3"
    ...
    <div class="flex flex-row flex-wrap justify-around p-5">
      <div
        v-for="elem in listingSearch()"
        :key="elem.id"
        class=" item-card flex p-5 bg-white mb-5"
      >
        <div v-if="elem.available" class="flex flex-col flex-wrap">
          <router-link :to="{ name: 'item-show', params: { id: elem.id } }"
        ...
        <div
          v-else
          class="unavailable-items-wrapper item-card flex flex-col flex-wrap"
        >
          <img :src="elem.image" alt="image-card" />
          <div class="flex flex-row justify-around pt-8">
            <h3>{{ elem.name }}</h3>
