<template>
  <div id="item-show" class="bg-red-200 h-full">
    <div
      v-if="item"
      class="show-page-wrapper flex flex-row justify-center h-full w-full bg-red-200 pt-5"
    >
      <div class="product-wrapper flex flex-row justify-center w-5/5 mb-20">
        <div
          class="flex flex-col w-3/5 bg-white items-start justify-center items-center mr-5 pr-5 pl-5"
        >
          <h1 class="item-show-header pr-5">{{ item.name }}</h1>
          <img :src="item.image" alt="item-image" />
          <h3 v-if="item.price" class="item-show-header">
            ${{ item.price.toFixed(2) }}
          </h3>
        </div>
        <div
          class="bg-red-100 h-full w-2/5 flex flex-col pl-5 pr-5 pt-10 pb-10 mr-5 justify-between text-gray-800"
        >
          <div class="details-wrapper h-full flex flex-col justify-start">
            <h4 class="title flex pb-1">
              Size:
              <p class="detail pl-1">{{ item.size }}</p>
            </h4>
            <div class="item-category-header flex flex-col">
              <p class="pr-1 title" v-if="item.categories">
                {{
                  `${item.categories.length > 1 ? "Categories" : "Category"}`
                }}:
              </p>
              <p
                class="detail detail-link pb-1"
          ...
          <div class="buttons-wrapper flex flex-col items-center pb-20">
            <div
              v-if="!isOwner"
              class="add-to-basket-wrapper flex justify-center mt-20"
            >
              <button
                v-if="item.available"
                class="button text-gray-900"
            ...
            <div v-if="isOwner" class="item-delete">
              ...
              <router-link :to="`/items/${item.id}/edit`">
                <button class="button text-gray-900">Edit this item</button>
              </router-link>
            </div>
          </div>
        </div>
      ...
      <div
        class="others-in-category flex flex-col items-start justify-start w-1/5 pt-2 mb-20 bg-red-300"
      >
        <div class="flex pt-3 pl-2">
          <h3
            class="title-others text-orange-600"
            v-if="othersInCategory.length"
          >
            Other items you might like:
          </h3>
          <h3 class="title-others text-orange-600 pl-5" v-else>
            More items coming soon!
          </h3>
        </div>
        <div
          v-if="othersInCategory"
          class="flex flex-col justify-start items-center h-full w-full pt-3"
        >
          <div
            v-for="item in othersInCategory"
            :key="item.id"
          ...
          >
            ...
            <div
              class="small-image-text flex pb-2 pt-1 justify-around items-center"
            >
    ...
    <div v-if="loginModal">
      <div class="login-modal flex" @click="loginModal = false">
        <div
          class="modal-text bg-white flex flex-col justify-around items-center text-orange-600 pl-5"
        >
          <div
          ...
          <div class="mb-20 text-gray-800 pb-3">
            <router-link to="/register">
              <button class="button text-gray-900 mr-2">Sign Up</button>
            </router-link>
            <router-link to="/login">
              <button class="button text-gray-900 ml-2">Login</button>
            </router-link>
          </div>
        </div>
      </div>
  ...
  methods: {
    async getItem() {
      try {
        ...
        this.othersInCategory = othersInCategory[0].filter(
          item => item.available
        );
      } catch (error) {
