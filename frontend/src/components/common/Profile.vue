<template>
  <div id="profile" class="flex flex-col items-center w-full h-full bg-red-200">
    <h1
      class="animated zoomInDown user-form-title text-6xl tracking-wide text-orange-600 mb-5"
    >
      Profile Page
    </h1>
    <div
      v-if="user"
      class="profile-wrapper flex w-full h-full justify-center items-center bg-red-200"
    >
      <div
        class="profile-name flex w-10/12 justify-center h-full p-5 bg-red-100 text-gray-800"
      >
      ...
      <div v-for="review in user.recommendations" :key="review.id">
        ...
        <div class="listings-wrapper flex flex-col w-1/3 items-center">
          ...
          <div
            ...
            :key="listing.id"
            class="flex flex-col items-center listing bg-white p-3 mb-3"
          >
            <p>{{ listing.name }}</p>
            <router-link :to="`/items/${listing.id}`"
        ...
        <div
          class="purchased-items-wrapper flex flex-col w-1/3 h-full items-center"
        >
          ...
          >
            ...
            <router-link :to="`/profile/${purchase.owner.id}/`"
              ><p>Sold by: {{ purchase.owner.username }}</p></router-link
            >
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    async getPurchasedItems() {
      try {
        ...
        this.purchasedItems = res.data.filter(
          item => item.purchased === this.user.id
        );
      } catch (err) {
        this.$router.push("/notfound");
      }
